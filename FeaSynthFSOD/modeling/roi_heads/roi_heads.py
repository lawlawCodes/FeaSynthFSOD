...
@ROI_HEADS_REGISTRY.register()
class SynFeatRes5ROIHeads(Res5ROIHeads):
    def __init__(self, cfg, input_shape):
        super().__init__(cfg, input_shape)
        ...
        self.base_index = [i for i in range(self.num_classes) if i not in self.novel_index]
        ...
        self.decoupled = getattr(cfg.MODEL.ROI_HEADS, "DECOUPLED", False)
        if self.decoupled:
    ...
    def forward(self, images, features, proposals=None, targets=None):
        del images
        ...
        if self.decoupled:
            feature_pooled_cls = F.relu(self.fc_s(feature_pooled))
            feature_pooled_loc = F.relu(self.fc_l(feature_pooled))
            pred_class_logits, pred_proposal_deltas = self.box_predictor(
                feature_pooled_cls, feature_pooled_loc
            )
        else:
        ...
        if self.training:
            ...
            if self.memory and hasattr(outputs, 'gt_classes'):
                pos_mask = outputs.gt_classes != self.num_classes
                if pos_mask.any():
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, keys_s, gt_class, queue):
        keys_s = keys_s[:self.queue_len]
        batch_size = keys_s.shape[0]
        ptr = int(self.queue_ptr[gt_class])
        
        if ptr + batch_size <= self.queue_len:
            queue[gt_class, ptr:ptr + batch_size] = keys_s
        else:
            overflow = ptr + batch_size - self.queue_len
            queue[gt_class, ptr:] = keys_s[:self.queue_len - ptr]
            queue[gt_class, :overflow] = keys_s[self.queue_len - ptr:]
            
        if ptr + batch_size >= self.queue_len:
            self.queue_full[gt_class] = 1
        self.queue_ptr[gt_class] = (ptr + batch_size) % self.queue_len

    @torch.no_grad()
    def update_memory(self, features, gt_classes, queue):
        ...
        for c in torch.unique(gt_classes_fg):
            c = int(c)
